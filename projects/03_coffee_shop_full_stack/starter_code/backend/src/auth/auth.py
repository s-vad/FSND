...
AUTH0_DOMAIN = 'coffee-shop-sv.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'coffee'
...
def get_token_auth_header():
    ...
    if parts[0].lower() != 'bearer':
        ...
        raise AuthError({
            'code': 'invalid_header',
            'description': 'JSON Web Token not found.'
        }, 401)

    elif len(parts) > 2:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header contains invalid token.'
        }, 401)
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            ...
            'description': 'Authorization details not included in JWT.'
        }, 400)

    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'invalid_authorization',
            'description': 'JWT does not contain permission.'
        }, 401)
